...
Why an ~org~ file? My Emacs config is large, and this enables me to improve the readability of its documentation greatly. I edit this file the same way you're reading it (nicely formatted) as I use Emacs and Emacs speaks ~org~. Sounds complicated, but it's really not; [[https://github.com/trinitronx/nixos-config/blob/main/hosts/nixos/default.nix#L236][just a few lines]] in my Nix config. [[https://github.com/trinitronx/nixos-config/blob/main/hosts/darwin/default.nix#L40][MacOS too.]]

This is the main configuration, but one more init file also exists, ~init.el~ , that bootstraps ~org-mode~ before this file is interpreted. That's defined [[https://github.com/trinitronx/nixos-config/blob/main/modules/shared/files.nix#L56][here]].
...
** Managing projects
*** Deadgrep
Deadgrep provides a full-buffer interface for ripgrep with excellent UX. Results are displayed in a dedicated buffer with syntax highlighting.
...
*** Dumb Jump - Universal Code Navigation
Dumb Jump provides a simple, language-agnostic way to jump to definitions using ag, rg, grep, or git-grep.
It works as a fallback when LSP is not available or for languages without LSP support.
